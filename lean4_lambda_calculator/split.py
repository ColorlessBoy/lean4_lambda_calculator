...
def split(target: Expr, proof: Expr, context: list[VarType], type_pool: dict[str, Expr]):
    if isinstance(proof, Lambda):
        assert isinstance(target, Forall), "proof is Lambda, but target is not Forall"
        var_type, _ = calc(proof.var_type, context, type_pool)
        assert var_type == target.var_type, f"Type mismatch: want\n  {target.var_type}\nget\n  {proof.var_type}\n\n"
        shifted_var_type = shift_expr(proof.var_type)
        shifted_context = [(BoundVar(0), shifted_var_type)] + shift_context(context)
        action, next_states = split(target.body, proof.body, shifted_context, type_pool=type_pool)
        ...
        if isinstance(proof.func, Const) or isinstance(proof.func, BoundVar):
            if isinstance(proof.arg, Const) or isinstance(proof.arg, BoundVar):
                return proof, []
            arg, arg_type = calc(proof.arg, context, type_pool)
            arg_action, next_states = split(arg_type, proof.arg, [], type_pool)
            return App(proof.func, arg_action), next_states
        func, func_type = calc(proof.func, context, type_pool)
        if isinstance(proof.arg, Const) or isinstance(proof.arg, BoundVar):
            func_action, next_states = split(func_type, proof.func, [], type_pool)
